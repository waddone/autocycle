    ...
    <div class="container NormalPageWhiteCont">
	    ...
	    <div class="row nopadding">
		    ...
		    <div class="col-xs-12 col-md-10 BlockDrCont">
		        <h3>Generator anunturi - varianta noua</h3>
		        @if(Session::has('message'))
				<p class="alert alert-info">{{ Session::get('message') }}</p>
				@endif
		    </div>
		    <div class="col-xs-12 col-md-10">
		        <form class="form-horizontal" method="POST" action="{{ route('adauga-generator-nou') }}" enctype="multipart/form-data">
		            {{ csrf_field() }}
                    ...
                    <div class="col-xs-12"> 
                    	...
                    	<h4>Lista piese</h4>
                    	<br/> 
                    	<? foreach ($piese_gen as $piesa_r) { ?>
                    	<div class="type-details">
                    		<div class="">
							    <label>
							      	<input type="checkbox" name="piesaId_<?=$piesa_r->id?>" class="checkMe" value="<?=$piesa_r->piesa?>">
							      	<?=$piesa_r->piesa?>
							    </label>
							</div>
							<div class="col-md-12 ToShow">
			                	<label>
							      	Titlu <?=$piesa_r->piesa?>
							    </label>
							    <input class="form-control" type="text" name="titlu_<?=$piesa_r->id?>" value="">
							    <label>
							      	Descriere <?=$piesa_r->piesa?>
							    </label>
			                	<textarea class="form-control" name="desc_<?=$piesa_r->id?>"></textarea>
			                	<label>
							      	Pret <?=$piesa_r->piesa?>
							    </label>
			                	<input class="form-control" type="text" name="pret_<?=$piesa_r->id?>" value="">
			                	<label>
							      	Moneda <?=$piesa_r->piesa?>
							    </label>
			                	<select name="moneda_<?=$piesa_r->id?>" class="form-control">
                    		...
                    		$(document).ready(function () {
							    ...
							    $('.checkMe').click(function() {
								  	if ($(this).prop('checked')) {
								    	$(this).parents('.type-details').find('.ToShow').fadeIn('slow');
								  	} else
								    	$(this).parents('.type-details').find('.ToShow').fadeOut('slow');
									});
							});
                    	</script>
                    </div>
